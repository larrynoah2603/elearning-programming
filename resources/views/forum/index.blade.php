...
<div class="py-8">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        <div class="surface-glow rounded-2xl p-6 bg-gradient-to-r from-secondary-50 via-white to-primary-50">
        ...
        <div class="surface-glow rounded-2xl p-6">
            ...
            <form method="POST" action="{{ route('forum.threads.store') }}" class="space-y-3">
                @csrf
                <input type="text" name="title" class="w-full rounded-xl border-gray-200 focus:border-primary-400" placeholder="Titre du sujet" required>
                <textarea name="body" rows="4" class="w-full rounded-xl border-gray-200 focus:border-primary-400" placeholder="Votre question..." required></textarea>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <select name="lesson_id" class="w-full rounded-xl border-gray-200">
                    ...
                    <select name="exercise_id" class="w-full rounded-xl border-gray-200">
        ...
        <div class="space-y-4">
            @forelse($threads as $thread)
                <div class="surface-glow rounded-2xl p-6 card-float">
                    <div class="flex justify-between gap-3">
                        ...
                        <div>
                            <form method="POST" action="{{ route('forum.threads.resolve', $thread) }}">
                                @csrf
                                @method('PATCH')
                                <button class="text-xs px-3 py-1.5 rounded-lg {{ $thread->is_resolved ? 'bg-success-100 text-success-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }} transition">
                    ...
                    <div class="mt-4 space-y-2">
                        @foreach($thread->replies as $reply)
                            <div class="p-3 bg-gray-50 rounded-xl text-sm border border-gray-100">
                    ...
                    <form method="POST" action="{{ route('forum.replies.store', $thread) }}" class="mt-3 flex gap-2">
                        @csrf
                        <input type="text" name="body" class="flex-1 rounded-xl border-gray-200" placeholder="Répondre..." required>
                        <button class="px-3 py-2 bg-gray-900 text-white rounded-xl hover:bg-black transition">Envoyer</button>
                    </form>
                </div>
            @empty
