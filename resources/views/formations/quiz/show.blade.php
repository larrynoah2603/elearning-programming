...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="bg-white rounded-xl shadow-sm p-8 border border-gray-100">
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-600 mb-4">{{ $quiz->description ?? 'Répondez aux questions suivantes pour valider vos connaissances.' }}</p>
            ...
            <div class="flex flex-wrap gap-4 text-sm">
                ...
                <span class="bg-purple-50 text-purple-700 px-3 py-1 rounded-full">
                    Tentative {{ $attemptCount + 1 }}/{{ $quiz->max_attempts }}
                </span>
                @if($quiz->duration_minutes)
                    <span class="bg-gray-50 text-gray-700 px-3 py-1 rounded-full">
                        ⏱️ {{ $quiz->duration_minutes }} minutes
                    </span>
                @endif
            </div>
        </div>
        ...
        <form id="quiz-form" action="{{ route('quiz.submit', $quiz) }}" method="POST">
            ...
            <div class="space-y-8">
                @foreach($quiz->questions->sortBy('order') as $index => $question)
                    <div class="question-item border border-gray-200 rounded-lg p-6" data-question-id="{{ $question->id }}">
                        ...
                        <div class="ml-11">
                            @switch($question->type)
                                    ...
                                    <div class="space-y-3">
                                        @foreach($question->answers->sortBy('order') as $answer)
                                            <label class="flex items-center gap-3 cursor-pointer">
                                                <input type="radio"
                                                       name="answers[question_{{ $question->id }}]"
                                                       value="{{ $answer->id }}"
                                ...
                                @case('essay')
                                    <div class="space-y-3">
                                        <textarea name="answers[question_{{ $question->id }}]"
                                                  rows="4"
