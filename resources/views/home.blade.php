        ...
        <div class="grid lg:grid-cols-2 gap-12 items-center">
            <div class="animate-fade-in">
                ...
                <div class="flex flex-col sm:flex-row gap-4">
                    <a href="{{ route('exercises.index') }}" class="group btn btn-primary bg-white text-primary-700 hover:bg-gray-50 hover:shadow-lg hover:shadow-white/20 hover:-translate-y-1 transition-all duration-300 px-8 py-4 text-lg rounded-xl font-semibold flex items-center justify-center">
                ...
                <div class="grid grid-cols-3 gap-8 mt-12 pt-8 border-t border-white/20">
                    ...
                    <div class="hover:scale-105 transition-transform duration-300 cursor-default">
                        <div class="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-br from-white to-white/70">{{ $stats['exercises'] ?? '200+' }}</div>
...
<section class="py-24 bg-gray-50 border-y border-gray-200/50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row justify-between items-end mb-12">
            ...
            <a href="{{ route('exercises.index') }}" class="group mt-4 md:mt-0 text-primary-600 hover:text-primary-700 font-semibold flex items-center transition-colors">
                    ...
                    <div class="group bg-white rounded-2xl shadow-sm hover:shadow-xl border border-gray-100 hover:border-primary-100 transition-all duration-300 hover:-translate-y-1 overflow-hidden flex flex-col">
                        <div class="p-6 flex-grow">
                            <div class="flex items-center justify-between mb-4">
                                <span class="badge badge-{{ $exercise->difficulty_badge_color }} px-3 py-1 rounded-full text-xs font-bold tracking-wide">
                                    {{ $exercise->difficulty_display }}
                                </span>
                                <i class="fab fa-{{ $exercise->programming_language }} text-3xl text-gray-300 group-hover:text-{{ $exercise->programming_language }}-500 transition-colors duration-300"></i>
                            </div>
                            <h4 class="font-bold text-gray-900 mb-2 text-lg group-hover:text-primary-600 transition-colors">{{ $exercise->title }}</h4>
                            <p class="text-gray-500 text-sm mb-6 line-clamp-2">{{ Str::limit($exercise->description, 100) }}</p>
                        </div>
                        <div class="px-6 py-4 border-t border-gray-50 bg-gray-50/50 flex items-center justify-between mt-auto">
                            <span class="text-sm font-semibold text-gray-700 flex items-center">
                                <i class="fas fa-star text-yellow-400 mr-1.5"></i> {{ $exercise->points }} pts
                            </span>
                            <a href="{{ route('exercises.show', $exercise->slug) }}" class="text-primary-600 hover:text-primary-800 text-sm font-bold flex items-center">
                ...
                @forelse($featuredLessons ?? [] as $lesson)
                    <div class="group bg-white rounded-2xl shadow-sm hover:shadow-xl border border-gray-100 transition-all duration-300 hover:-translate-y-1 overflow-hidden">
                        ...
                        <div class="p-6">
                            ...
                            <h4 class="font-bold text-xl text-gray-900 mb-3 group-hover:text-primary-600 transition-colors">{{ $lesson->title }}</h4>
                            <p class="text-gray-500 text-sm mb-6 line-clamp-2 leading-relaxed">{{ Str::limit($lesson->description, 100) }}</p>
                            <a href="{{ route('lessons.show', $lesson->slug) }}" class="block w-full text-center bg-primary-50 text-primary-700 hover:bg-primary-600 hover:text-white font-semibold py-3 rounded-xl transition-colors duration-300">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($categories ?? [] as $category)
                <a href="{{ route('categories.show', $category->slug) }}" class="group bg-white border border-gray-100 shadow-sm hover:shadow-xl hover:border-primary-200 rounded-2xl p-6 flex items-center space-x-5 transition-all duration-300 hover:-translate-y-1">
                    <div class="w-16 h-16 bg-gray-50 group-hover:bg-primary-50 rounded-2xl flex items-center justify-center transition-colors duration-300">
                        <i class="fas {{ $category->icon ?? 'fa-code' }} text-3xl text-gray-400 group-hover:text-primary-600 group-hover:scale-110 transition-all duration-300"></i>
                    </div>
                    <div>
                        <h3 class="font-bold text-lg text-gray-900 group-hover:text-primary-600 transition-colors">{{ $category->name }}</h3>
                        <p class="text-gray-500 text-sm mt-1 flex items-center">
                            <i class="fas fa-layer-group text-xs mr-2 opacity-50"></i> {{ $category->lessons_count }} leçons
                        </p>
                    </div>
                </a>
            @empty
            ...
            <div class="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-shadow duration-300 p-8 relative overflow-hidden">
                ...
                <div class="relative z-10">
                    <div class="flex text-yellow-400 mb-6 text-sm">
                        @for($i = 0; $i < 5; $i++) <i class="fas fa-star mr-1"></i> @endfor
                    </div>
